...
const sendMessageToWhatsApp = async (phoneNumber, aiResponce) => {
    try {
        let data;
        const {resp, type} = aiResponce;
        console.log(aiResponce,'aiResponceaiResponce')
        if (type === "list") {
        ...
        await axios.post(`${baseUrl}/${whatsappPhoneId}/messages`, data, config);
    } catch (error) {
